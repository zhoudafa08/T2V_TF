...
def evaluate_trading_fee(prediction, ground_truth, mask, report=False):
    ...
    pre_rand5 = set()
    pre_rand10 = set()
    for i in range(prediction.shape[1]): #date
        rank_gt = np.argsort(ground_truth[:, i])
        gt_top1 = set()
        gt_top5 = set()
        gt_top10 = set()
        trading_fee_long = 0.0
        trading_fee_long5 = 0.0
        trading_fee_long10 = 0.0
        trading_fee_longall = 0.0
        trading_fee_rand = 0.0
        trading_fee_rand5 = 0.0
        trading_fee_rand10 = 0.0
        for j in range(1, prediction.shape[0] + 1): #stock
        ...
        rank_pre = np.argsort(prediction[:, i]) #stock id ordered
        ...
        for j in range(1, prediction.shape[0] + 1): #stock
            ...
            #    continue
            if len(tmp_set1) < 1:
                tmp_set1.add(cur_rank)
            if len(tmp_set5) < 5:
                tmp_set5.add(cur_rank)
            if len(tmp_set10) < 10:
                tmp_set10.add(cur_rank)
            tmp_setall.add(cur_rank)
        
        trading_count = len(tmp_set1 - pre_top1) * 2
        trading_fee_long = trading_count * trading_fee
        pre_top1 = tmp_set1
        trading_count = len(tmp_set5 - pre_top5) * 2
        trading_fee_long5 = (trading_count * trading_fee / 5)
        pre_top5 = tmp_set5
        trading_count = len(tmp_set10 - pre_top10) * 2
        trading_fee_long10 = (trading_count * trading_fee / 10)
        pre_top10 = tmp_set10
        trading_count = len(tmp_setall - pre_topall) * 2
        trading_fee_longall = (trading_count * trading_fee / prediction.shape[0])
        pre_topall = tmp_setall
        ...
        tmp_set1.add(rank_pre[sid[0]])
        sid = np.random.choice(prediction.shape[0], 5, replace=False)
        for id in sid:
            tmp_set5.add(rank_pre[id])
        sid = np.random.choice(prediction.shape[0], 10, replace=False)
        for id in sid:
            tmp_set10.add(rank_pre[id])
        trading_count = len(tmp_set1 - pre_rand1) * 2
        trading_fee_rand = trading_count * trading_fee
        pre_rand1 = tmp_set1
        trading_count = len(tmp_set5 - pre_rand5) * 2
        trading_fee_rand5 = (trading_count * trading_fee / 5)
        pre_rand5 = tmp_set5
        trading_count = len(tmp_set10 - pre_rand10) * 2
        trading_fee_rand10 = (trading_count * trading_fee / 10)
        pre_rand10 = tmp_set10
        ...
        bt_longall -= trading_fee_longall
